...
reflector -c US --latest 20 --sort rate --download-timeout 60 --connection-timeout 60 --save /etc/pacman.d/mirrorlist
...
chmod a+x /mnt/root/configtest.sh


# change root into the new system and run config script
arch-chroot /mnt /root/configtest.sh #might need to remove /root/config here to get config script to run
